...
PRINT [items]   display values on the screen
  Separators control spacing:
    ...
    ,        print spaces up to the next 16-column zone (4 zones per
...
GOTO n   jump to line n and continue from there
  ...
  The number is read as the machine reads it: GOTO with nothing behind
  it, or with something that is not a number, is GOTO 0, and a number
  past 65529 is ?SN.  GOSUB, RUN n and THEN n take theirs the same way.
...
GOSUB n   call the subroutine beginning at line n
  ...
  A subroutine cannot see its caller's FOR loops (the ROM's scan of the
  stack stops at the GOSUB).  FOR I inside it opens a NEW loop even while
  the caller is in its own FOR I -- the variable I is still shared, the
  loop is not -- and a NEXT there for a loop opened before the GOSUB is
  ?NF.  RETURN discards any loop the subroutine left open.
...
INPUT ["prompt";] var[,var...]   read values typed at the keyboard
  Waits for a line and ENTER.  The prompt is followed by ; and BASIC
  adds "? " behind it; a , there is ?SN, as on the machine (LINE INPUT
  takes only ; too).
  ...
  A typed line ends where a DATA statement would: a colon outside quotes
...
CLEAR [n]   reset all variables; optionally set string space to n bytes
  ...
  comes FIRST, then DEFSTR and ON ERROR GOTO, as period programs have it:
...
LPRINT list   the same as PRINT, but to the printer
  ...
  A path that cannot be written -- a directory, a folder that is not
  there, a file without permission -- is ?FD at the first LPRINT or
  LLIST, with a line naming the path.  A device is a fine printer:
  /dev/null discards, /dev/stdout shows the printout in a capture.
...
LLIST [n][-[m]]   list program lines to the PRINTER
  ...
  check the destination before assuming the program is at fault.  Here
  the printer is the TRS80_PRINTER file (see: man LPRINT); a path that
  cannot be written is ?FD.
  Example: LLIST
...
OUT p,v   write value v to Z80 port p
  ...
  INP(255) reads 63 or 127 accordingly.  Period programs flash the screen
  by toggling it in a loop.  Only the hardware changes: the ROM's print
  routines step the cursor by their own flag, bit 3 of 16445 (403DH),
  which CHR$(23) sets and CLS clears, so OUT 255,8 alone shows every
  other character of what is printed next, as on the machine.  Barden's
  recipe for readable 32-column text from BASIC is OUT 255,8 with
  POKE 16445,8.  The other bits of port 255 (the cassette signal and
  relay) and every other port are accepted and discarded: both
  expressions are evaluated (so a type error still reports) and no other
  port hardware is simulated.  INP is its counterpart.
  Example: OUT 255,4
```
...
RUN "name"   load that file and run it (Disk BASIC)
  ...
  resume part-way through with the variables intact.  A number that
  names no line is ?UL (RUN 0 with no line 0, RUN 10 in an empty
  program), where plain RUN in an empty program is simply READY.
  Example: RUN
  Example: RUN 500
```
...
LIST [n][-[m]]   display program lines on the screen
  LIST ends at READY, as on the machine: inside a program it lists and
  then STOPS the program (so do LLIST, DELETE, AUTO, CLOAD and LOAD
  without ,R -- a program that CLOADs its next part must be RUN again).
...
LOAD "name"   replace the program in memory with one from a file
  ...
  Reads the plain-text form written by SAVE.  The lines of a text
  ...
  A file that is not there is ?FF, File not found (54, ERR=106), for
  LOAD, RUN "file" and MERGE alike -- the code OPEN and KILL give, and
  the one a Disk BASIC listing tests for.  CLOAD of a missing file is
  ?FD: a tape has no "not found".
...
NAME [n[,[m][,i]]]   renumber the program (Disk BASIC)
  ...
  An ON ERROR GOTO already in force follows its line too.  The FOR and
  GOSUB stacks are dropped and CONT is off, as after any change to the
  program.
...
ERR   the last error's code, held as (code-1)*2
  ...
  The codes, 1-23 as the Level II manual's appendix B lists them:
     1 NF NEXT without FOR      13 TM Type mismatch
     2 SN Syntax error          14 OS Out of string space
     3 RG RETURN without GOSUB  15 LS String too long
     4 OD Out of data           16 ST String formula too complex
     5 FC Illegal function call 17 CN Can't continue
     6 OV Overflow              18 NR No RESUME
     7 OM Out of memory         19 RW RESUME without error
     8 UL Undefined line        20 UE Unprintable error
     9 BS Subscript out of range 21 MO Missing operand
...
PEEK(addr)   read a memory byte (unset = 255)
  ...
  16445 (403DH) is the ROM's image of the port-255 bits: bit 3 is its
  32-column print flag, set by CHR$(23), cleared by CLS, and POKEable, as
  Barden's OUT 255,8 with POKE 16445,8 recipe does (see OUT).
...
OPEN mode$, [#]n, name$ [,reclen]   open a file on channel n (1-15)
  ...
  SYSTEM and KILL: a listing cannot open a socket or load a program from
  one.  The same goes by what the name IS, not how it is spelled: "I"
  reads a regular file, the other modes write a regular file or a name
  that does not exist yet, so a device reached another way (//dev/zero),
  a directory or a FIFO is ?FD too.  A symbolic link to a file is that
  file.
...
KILL name$   delete a file from the host disk
  The file must not be open -- CLOSE it first, or ?AO.  There is no
  confirmation and no recovery.
  Deleting a file that does not exist raises ?FF (file not found), so
...
INPUT #n, vars   read items from a file opened "I"
  ...
  The number is read the way VAL reads it: text that is no number gives
  0 and "5X" gives 5 -- a file never raises ?TM.
...
LINE INPUT #n, v$             read one whole line from a file
  ...
  A string holds 255 characters, so LINE INPUT #n stops at the 255th and
  leaves the rest of the line for the next read; INPUT #n ends an item
  there too.
  A CHR$(13) in the file ends a record wherever it falls, as it does on
  the machine -- so a file written with PRINT #n,A$;CHR$(13);B$ reads
  back as two records, and a data file whose lines end in CR alone is
...
FIELD [#]n, w AS v$ [, w AS v$...]   name slices of a random record
  ...
  variables (array elements too: 16 AS C$(1)).  The widths must not
  exceed the record length, or ?FO.
...
GET [#]n [,rec]   read a record from a random file into the buffer
  ...
  Reading past the end gives a record of zero bytes, CHR$(0), rather than
  an error (as the Disk manual says), so check LOF first; EOF(n) is true
  after such a GET.
  Example: GET 1, 5
...
```text
LOF(n)   the number of records in the file on channel n
  ...
  It serves a sequential file too ("I", "O" or "E"), as the Disk manual
  says it does.  Such a file has no record length of its own, so its
  records are the machine's 256-byte physical ones and LOF is the file's
  length rounded up to whole records.  Asking does not move the read.
...
EXP(x)   e raised to the power x -- the inverse of LOG
  ...
  The range is the ROM's, and it is narrower than the number format:
  x of 87.3365 or more is ?OV, though the answer there is only 8.5E+37,
  half of what a number can hold -- the routine scales its result by a
  power of two and runs out of exponent first.  x of -88.7228 or less
  gives 0: the ROM tests the sign before it reports the overflow.
...
```text
CINT(x)   the largest integer not greater than x, as a 16-bit integer
  Level II's CINT rounds DOWN, like INT -- it does not round to nearest:
  CINT(1.5) is 1 and CINT(-1.5) is -2 (the manual's own examples).  The
  difference from INT is the range: x must satisfy -32768 <= x < 32768,
  or ?OV.  AND, OR and NOT convert their operands the same way, which is
  why V/16 AND 15 gives the high hex digit of V.
  Example: PRINT CINT(3.7);CINT(-3.7);CINT(32767.9)   ->  3 -4  32767
```
...
CSNG(x)   convert x to single precision
  Rounds x to a single's 24 bits (half up on the magnitude, as the ROM
  rounds at 0796H) and returns a single: CSNG(1#/3) is .3333334, and
  printed to 6 digits, .333333.  Every number carries its type -- an
  integer, a single or a double -- and prints by it: an integer in
  full, a single to 6 significant digits (1000000 is 1E+06), a double
  to 16 with D as its exponent letter (1D+20).  A literal is typed as
  ...
  eighth or with a D exponent or #.  An operation takes the wider
...
CDBL(x)   convert x to double precision
  Returns x as a double: the value is not made more precise than it
  was, so CDBL(1/3) is the single quotient .3333333432674408, and only
  double operands (1#/3, A#/B#) give a double result.  A double holds
  53 bits here against the machine's 56, so the sixteenth printed digit
  of a long fraction can differ from the machine's (2#/3 prints
  .6666666666666666; the machine .6666666666666667); everything a
  program can see in 15 digits is the same.
...
VAL(a$)   the number at the front of a$, or 0 if there is none
  ...
  first character that does not fit, so VAL("12.5X") is 12.5.  Spaces
  inside the number are skipped, as the ROM skips them: VAL("1 2") is 12.
  Exponent notation is understood: VAL("-3E2") is -300.  A sign counts
  only as the very first character, so VAL(" -5") is 0.  A trailing ! or
  # is accepted; a trailing % is ?SN, as on the machine.
...
POS(n)   the column the cursor is currently on, counting from 0
  ...
  In 32-character mode it is the character column, 0-31.
...
INKEY$   the key being pressed right now, or "" if none
  ...
  (SHIFT + up arrow on the machine).  The Delete/Backspace key is the
  left arrow, CHR$(8): that IS the machine's backspace, and no Model I
  key produces 127.  Keys the TRS-80 does not have -- PgUp, the function
  keys -- return nothing.
