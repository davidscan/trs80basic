...
# (thr_wait, p10).  Until 2026-09-26 each slice forked `sleep` for its
# whole share on top of the time already spent, and the same loop ran 40%
# slow (the 2026-09-23 audit, L-13).  The loop here is 4003 statements:
# 2.26 s nominal, 3.2 s under the old code, so the bound below separates
# them with room for a slow runner.
# The closed loop is measured with the extension loaded HERE (gawk -l),
# its stderr discarded, so the case runs wherever the extension loads at
# all -- CI's Ubuntu gawk loads it with a line on stderr, which is why the
# launcher does without it there (kbd_pty.py covers the launcher's
# choice).  Without an extension that loads, the pacing is open-loop and
# only "not faster" is asserted; the clock case then prints SKIPPED,
# which run_all counts.
...
# the clock: an extension that loads and answers gettimeofday()
tm=
for ext in time timex; do
    if [ "$(gawk -l "$ext" 'BEGIN { print (gettimeofday() > 0) }' 2>/dev/null)" = "1" ]; then tm="-l $ext"; break; fi
done
now() { gawk $tm 'BEGIN { printf "%.3f", gettimeofday() }' 2>/dev/null; }
...
# the interpreter with the clock, as the launcher runs it where the
# extension is silent; the extension's own stderr line, if any, is dropped
withclock() { TRS80_DUMB=1 TRS80_Z80= gawk -b $tm -f "$here/trs80basic.awk" -- "$@" 2>/dev/null; }
...
if [ -z "$tm" ]; then
    echo "SKIPPED: the closed-loop pacing (no time extension loads in this gawk)"
    rm -rf "$tmp"; echo "SPEED OK"; exit 0
fi

# 2. the closed loop: within 5% below and 15% (plus start-up) above the
# nominal.  The old code's 3.2 s fails this.
t0=$(now)
out=$(cd "$tmp" && TRS80_MHZ=1.77 withclock loop.bas)
t1=$(now)
e=$(elapsed "$t0" "$t1")
