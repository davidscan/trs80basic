...
189 REM CHR$(28), home, also returns to 64 characters per line (ROM 04C0-04CD): the flag, the latch and the step
190 CLS:PRINT CHR$(23);"MN";CHR$(28);:IF PEEK(16445)<>0 OR INP(255)<>127 THEN PRINT "FAIL CHR$(28) returns to 64 columns";PEEK(16445);INP(255):F=1
192 PRINT "OP";:IF PEEK(15360)<>79 OR PEEK(15361)<>80 THEN PRINT "FAIL CHR$(28) homes with the 1-byte step";PEEK(15360);PEEK(15361):F=1
195 OUT 255,0:CLS
200 IF F THEN PRINT "OUT 255 FIXTURE FAILED":ERROR 5
