...
for b in varptr rawbytes alias inp out255 ifcomma intconv forstack pokerange mbfpoke round errcode cursor dataitem power using; do
...
for s in break devvec usr pmtrunc z80 z80core sound tokload system tips_probe hostwrite special linelen clear memsize print input ready inputnum printcomma onerror ollama randfile lof crunch auto notty corepath errline goto imgpoke lineedit numov numread inputitem freshline linecut dotline fname; do
