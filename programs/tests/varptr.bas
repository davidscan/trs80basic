...
60 V=VARPTR(A$):REM the ROM's integer, negative above 32767 (L-43); PEEK and POKE take only that form (M-11)
62 IF PEEK(V)<>10 THEN PRINT "FAIL len byte":F=1
65 REM a data address composed from two bytes is positive: the period idiom makes it the integer again
70 D=PEEK(V+1)+256*PEEK(V+2):D=D+65536*(D>32767):IF D<>V-10 THEN PRINT "FAIL data just below descriptor":F=1
80 REM the two-call period idiom must compose the real data address
90 D2=PEEK(VARPTR(A$)+1)+256*PEEK(VARPTR(A$)+2):D2=D2+65536*(D2>32767):IF D2<>D THEN PRINT "FAIL two-call idiom";D2;D:F=1
...
120 B$=STRING$(200,65)
130 P=PEEK(VARPTR(B$)+1)+256*PEEK(VARPTR(B$)+2):P=P+65536*(P>32767):FOR I=0 TO 199:POKE P+I,66:NEXT
...
200 A$=A$+"XYZ":V2=VARPTR(A$):IF V2<>V THEN PRINT "FAIL descriptor moved on growth":F=1
210 IF PEEK(V)<>13 THEN PRINT "FAIL len after growth":F=1
220 D3=PEEK(V+1)+256*PEEK(V+2):D3=D3+65536*(D3>32767):IF PEEK(D3+12)<>90 THEN PRINT "FAIL data after growth";PEEK(D3+12):F=1
230 REM shrinking never moves anything
240 A$="AB"+"":V2=VARPTR(A$):P=PEEK(V+1)+256*PEEK(V+2):P=P+65536*(P>32767):IF V2<>V OR P<>D3 THEN PRINT "FAIL shrink moved data":F=1
...
310 P=PEEK(16526)+256*PEEK(16527):P=P+65536*(P>32767):IF PEEK(P)<>66 THEN PRINT "FAIL vector via two-call":F=1
312 REM a kept VARPTR follows a string that GROWS by assignment: the descriptor names the new bytes at once
314 G$="AB":V=VARPTR(G$)
315 G$="ABCDEFGHIJ":D=PEEK(V+1)+256*PEEK(V+2):D=D+65536*(D>32767)
...
318 POKE D+9,90:V2=VARPTR(G$):IF G$<>"ABCDEFGHIZ" OR V2<>V THEN PRINT "FAIL POKE into the grown string: ";G$:F=1
...
320 H$="ABCDEFGH"+"":V=VARPTR(H$):D=PEEK(V+1)+256*PEEK(V+2):D=D+65536*(D>32767):H$="AB"+"":POKE D+5,88
