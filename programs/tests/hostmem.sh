...
printf '10 ABCD=5:PRINT AB;ABCD\n' > "$tmp"
out=$(host); [ "$out" = " 0  5 " ] || fail "every character of a name counts (varnames.sh has the rest)" "$out"
out=$(run);  [ "$out" = " 5  5 " ] || fail "the machine keeps two characters of a name (unchanged)" "$out"
