...
# the file error codes the man pages name are the ones the code raises.
# They did not match: `man OPEN` and `man KILL` said ?FO where both raise
...
BAS
out=$(cd "$dir" && TRS80_Z80= "$here/basic" e.bas 2>&1)
[ "$out" = "CODES OK" ] || fail "the file error codes the docs name" "$out"
